...
class DeblurProcessor:
    ...
    def process(self, image: np.ndarray, frame_idx: int = 0, output_path = None) -> np.ndarray:
        """处理图像散焦去模糊
        
        Args:
            image: 输入图像
            frame_idx: 帧索引
            output_path: 输出路径，用于保存中间结果
        ...
        if output_path is not None:
            self._save_morphology_result(result, frame_idx, output_path)
        
        self.logger.info(f"第{frame_idx}帧散焦去模糊处理完成")
        return result
    
    def _save_morphology_result(self, image: np.ndarray, frame_idx: int, output_path):
        """执行形态学开运算并保存结果
        ...
        try:
            # 创建保存目录
            morph_dir = Path(output_path) / "03_morphology"
            ...
            # 再开运算去除小物体
            opened = cv2.morphologyEx(closed, cv2.MORPH_OPEN, kernel, iterations=13)
            
            # 保存形态学处理结果
            morph_path = morph_dir / f"morph_{frame_idx:04d}.png"
            cv2.imwrite(str(morph_path), opened)
            self.logger.info(f"形态学开运算结果已保存: {morph_path}")
            
        except Exception as e:
            self.logger.error(f"保存形态学处理结果时出错: {str(e)}")
