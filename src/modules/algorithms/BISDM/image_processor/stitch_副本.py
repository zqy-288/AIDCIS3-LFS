...
class StitchProcessor:
    ...
    def _calculate_depth_positions(self):
        ...
        # 使用位移数据和毫米/像素比例计算深度位置
        mm_per_pixel = self.mm_per_pixel if self.mm_per_pixel is not None else 0.1
        ...
        for offset in self.offsets_data:
            ...
            offset = int(offset)
            # 计算深度位置
            depth_mm = self.initial_depth_mm + offset * mm_per_pixel
    ...
    def _add_legend(self, panorama: np.ndarray) -> np.ndarray:
        ...
        # 计算图像实际尺寸
        original_height = height - self.top_margin - (self.angle_axis_height if self.enable_angle_axis else 0)
        original_width = width - (self.depth_axis_width if self.enable_depth_axis else 0)
        
        # 设置文本内容（根据语言选择）
        if self.use_english_labels:
            # 英文标签
            image_size_text = f"Image Size: {original_width} x {original_height} px"
            pipe_length_text = f"Pipe Length: {total_length_mm:.2f} mm"
            scale_text = f"Scale: 1px = {pixel_to_mm_ratio:.4f} mm"
            scale_label = f"{10.0} mm"  # 初始值，可能会根据实际情况调整
        else:
            # 中文标签
            image_size_text = f"图像尺寸: {original_width} × {original_height} 像素"
            pipe_length_text = f"孔道长度: {total_length_mm:.2f} mm"
        ...
        try:
            ...
            # 绘制比例尺标签
            scale_label = f"{scale_length_mm} mm"
            # 重新创建PIL图像绘制最后的标签
            pil_img = Image.fromarray(panorama_vis)
            draw = ImageDraw.Draw(pil_img)
            ...
            cv2.putText(panorama_vis, f"Pipe Length: {total_length_mm:.2f} mm", 
                       (text_x, text_y), font, font_scale, self.legend_text_color, thickness)
            text_y += line_height
            ...
            cv2.line(panorama_vis, (scale_x1, scale_y-5), (scale_x1, scale_y+5), self.legend_text_color, 1)
            cv2.line(panorama_vis, (scale_x2, scale_y-5), (scale_x2, scale_y+5), self.legend_text_color, 1)
            cv2.putText(panorama_vis, f"{scale_length_mm} mm", 
    ...
    def _rotate_and_add_axes(self, panorama: np.ndarray, images: List[np.ndarray]) -> np.ndarray:
        ...
        if self.enable_legend:
            ...
            try:
                ...
                cv2.putText(expanded, f"Pipe Length: {total_length_mm:.2f} mm", 
                           (text_x, text_y), font, font_scale, self.legend_text_color, thickness)
                text_y += line_height
                ...
                cv2.line(expanded, (scale_x1, scale_y-5), (scale_x1, scale_y+5), self.legend_text_color, 1)
                cv2.line(expanded, (scale_x2, scale_y-5), (scale_x2, scale_y+5), self.legend_text_color, 1)
                cv2.putText(expanded, f"{scale_length_mm} mm", 
                          ((scale_x1 + scale_x2) // 2 - 15, scale_y + 15), 
                          font, font_scale, self.legend_text_color, thickness)
