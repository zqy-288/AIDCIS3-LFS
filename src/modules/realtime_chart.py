...
此文件将在未来版本中移除，请使用新的模块化版本：
from src.modules.realtime_chart_package import RealtimeChart

This file will be removed in future versions. Please use the new modular version:
from src.modules.realtime_chart_package import RealtimeChart
...
import warnings
warnings.warn(
    "realtime_chart.py is deprecated. Use 'from src.modules.realtime_chart_package import RealtimeChart' instead.",
    DeprecationWarning,
    stacklevel=2
)
...
class RealtimeChart(QWidget):
    ...
    def setup_ui(self):
        ...
        self.endoscope_view.setMinimumWidth(400)
        splitter.addWidget(self.endoscope_view)

        # 设置分割器比例，使用相对比例而不是固定像素
        # 面板A占50%，面板B占50% - 平均分配空间
    ...
    def init_hole_data_mapping(self):
        """初始化孔位数据映射"""
        # 避免重复初始化
        if hasattr(self, 'hole_to_csv_map') and self.hole_to_csv_map:
            return
            
        import os

        from pathlib import Path  # 确保在这个方法中导入Path
        # 使用绝对路径确保路径解析正确
        project_root = Path(__file__).parent.parent.parent
        data_path = project_root / "Data" / "CAP1000"
    ...
    def set_current_hole_display(self, hole_id):
        ...
        if hole_id:
            ...
            # 如果有对应的数据文件，自动加载
            if hole_id in ["AC002R001", "AC004R001", "BC001R001", "BC003R001"]:
    ...
    def adjust_splitter_sizes(self, splitter):
        ...
        if total_height > 0:
            # 面板A占50%，面板B占50% - 给内窥镜更多空间
            panel_a_height = int(total_height * 0.5)
            panel_b_height = int(total_height * 0.5)
            splitter.setSizes([panel_a_height, panel_b_height])
            
            # 确保内窥镜面板有足够的最小高度
            if panel_b_height < 300:
                panel_a_height = total_height - 300
                panel_b_height = 300
                splitter.setSizes([panel_a_height, panel_b_height])
    ...
    def init_data_buffers(self):
        ...
        from pathlib import Path  # 确保在这个方法中导入Path
        project_root = Path(__file__).parent.parent.parent
        self.csv_base_path = str(project_root / "Data" / "CAP1000" / "R001C001" / "CCIDM")
    ...
    def view_next_sample(self):
        ...
        # 定义孔位切换顺序
        hole_sequence = ["AC002R001", "BC001R001"]
