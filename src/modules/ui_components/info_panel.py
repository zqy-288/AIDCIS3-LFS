...
class InfoPanel(QWidget):
    ...
    def _create_progress_section(self, layout):
        ...
        for label in [self.completed_count_label, self.pending_count_label,
                     self.current_batch_label, self.batch_progress_label,
    ...
    def _create_file_info_section(self, layout):
        ...
        file_group_font = QFont()
        file_group_font.setPointSize(10)
        file_group_font.setBold(True)
    ...
    def update_batch_info(self, batch_name: str, current: int, total: int):
        """更新批次信息"""
        self.current_batch_label.setText(f"检测批次: {batch_name}")
    ...
    # 文件信息相关方法
    def update_file_info(self, file_name: str, file_size: str, hole_count: int, load_time: str = None):
        """更新文件信息"""
        self.file_name_label.setText(file_name)
        self.file_size_label.setText(f"大小: {file_size}")
        self.hole_count_label.setText(f"孔位数量: {hole_count}")
        if load_time:
            self.load_time_label.setText(f"加载时间: {load_time}")
    
    def clear_file_info(self):
        """清空文件信息"""
        self.file_name_label.setText("未加载文件")
        self.file_path_label.setText("路径: -")
        self.file_size_label.setText("大小: -")
        self.load_time_label.setText("加载时间: -")
        self.hole_count_label.setText("孔位数量: 0")
