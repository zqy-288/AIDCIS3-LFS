...
class DataFolderMonitor(QObject):
    ...
    def initial_scan(self):
        ...
        self.logger.info(f"📂 扫描路径: {self.data_root_path}")
        self.logger.info(f"📂 路径是否存在: {self.data_root_path.exists()}")
        
        try:
            if not self.data_root_path.exists():
                self.logger.error(f"❌ 数据路径不存在: {self.data_root_path}")
                return
...
def get_data_monitor() -> DataFolderMonitor:
    ...
    if _global_monitor is None:
        ...
        logger.info(f"🔧 数据路径存在: {data_path.exists()}")
        
        _global_monitor = DataFolderMonitor(str(data_path))
