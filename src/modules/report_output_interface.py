...
class ReportOutputInterface(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._last_workpiece_id = None
        
        self.setup_ui()
        self.setup_connections()
        # load_workpiece_list() 已在 setup_connections() 中的 load_available_workpieces() 调用，避免重复
    ...
    # load_workpiece_list() 方法已删除，功能合并到 load_available_workpieces()
    ...
    def load_workpiece_data(self, workpiece_id: str):
        ...
        try:
            ...
            # 收集工件数据 - 使用缓存避免重复收集
            report_data = self._get_cached_report_data(workpiece_id)
    ...
    def preview_report(self):
        ...
        try:
            # 收集报告数据 - 使用缓存避免重复收集
            report_data = self._get_cached_report_data(self.current_workpiece_id)
    ...
    def _get_cached_report_data(self, workpiece_id: str):
        """获取缓存的报告数据，避免重复收集"""
        # 检查是否是相同的工件ID和是否有缓存
        if workpiece_id == self._last_workpiece_id and workpiece_id in self._cached_report_data:
            print(f"🎯 [报告缓存] 使用缓存数据: {workpiece_id}")
            return self._cached_report_data[workpiece_id]
        
        # 收集新数据
        print(f"🔄 [报告缓存] 收集新数据: {workpiece_id}")
        report_data = self.report_generator.collect_workpiece_data(workpiece_id)
        
        # 更新缓存
        self._cached_report_data[workpiece_id] = report_data
    ...
    def _clear_report_cache(self):
        """清空报告数据缓存"""
        self._cached_report_data.clear()
        self._last_workpiece_id = None
        print("🧹 [报告缓存] 缓存已清空")
...
• Excel格式 (.xlsx) - 表格格式
    ...
    def load_available_workpieces(self):
        ...
        try:
            ...
            # 该工件包含20000个孔，目前有C001R001~C001R003的数据，后续会增加
            ...
            self.workpiece_combo.setToolTip(
                "工件 CAP1000:\n"
                "• 总孔位数: 20000个\n"
                "• 当前有数据的孔位: C001R001, C001R002, C001R003\n"
                "• 后续会增加更多孔位数据\n"
                "• 报告将包含所有已检测孔位的数据"
            )
            ...
            if self.workpiece_combo.count() > 0:
                ...
                # 手动触发一次数据加载，因为setCurrentIndex不会触发currentTextChanged信号
