...
QLabel#InfoLabel {{
    ...
    border-radius: 4px;
    font-weight: bold;
    font-size: 18px;  /* 增大字体从默认15px到18px */
}}
...
QPushButton#ClearDataButton {{
    ...
    border-radius: 4px;
    padding: 8px 16px;
    min-width: 140px;  /* 设置最小宽度确保文字完整显示 */
    height: 40px;      /* 设置固定高度保持一致性 */
}}
...
QLabel#StatusLabel {{
    ...
    padding: 6px 10px;
    border-radius: 4px;
    font-size: 25px;  /* 从13px增大到18px，与其他标签保持一致 */
    color: {cls.COLORS['text_primary']};
}}
...
QTabBar::tab {{
    ...
    margin-right: 2px;
    font-size: 16px;  /* 从13px增大到16px，提升选项卡文字可读性 */
}}
...
QWidget#Sidebar QGroupBox::title {{
    ...
    padding: 0 5px;
    background-color: {cls.COLORS['background_secondary']};
    color: {cls.COLORS['text_primary']}; /* 标题颜色改为更柔和的灰白 */
    font-weight: bold;
}}
