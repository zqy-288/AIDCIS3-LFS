...
# --- 1. 配置区域 ---
def find_psexec_path():
    """动态查找PSExec工具路径"""
    # 常见的PSExec安装位置
    possible_paths = [
        r"C:\PSTools\PsExec64.exe",
        r"C:\Program Files\PSTools\PsExec64.exe", 
        r"C:\Windows\System32\PsExec64.exe",
        r"D:\PSTools\PsExec64.exe",
        r"F:\PSTools\PsExec64.exe"
    ]
    
    for path in possible_paths:
        if os.path.exists(path):
            print(f"✅ 找到PSExec工具: {path}")
            return path
    
    # 如果都找不到，返回系统PATH中的psexec
    print("⚠️ 未找到PSExec工具，尝试使用系统PATH中的版本")
    return "psexec64.exe"

PSEXEC_PATH = find_psexec_path() 
