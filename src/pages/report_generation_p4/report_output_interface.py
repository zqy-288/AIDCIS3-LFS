        ...
        try:
            ...
            if self.config.report_format == ReportFormat.PDF and PDF_AVAILABLE:
                try:
                    ...
                    # PDF生成失败，回退到文本报告
                    print(f"❌ PDF生成失败，回退到文本报告: {pdf_error}")
                    txt_path = instance.output_path.replace('.pdf', '.txt')
                    self._generate_simple_report(report_data, txt_path)
                    # 更新输出路径为文本文件
                    instance.output_path = txt_path
            else:
                # 回退到简单文本报告
...
class ReportPreviewDialog(QDialog):
    ...
    def _generate_text_preview(self) -> str:
        ...
        lines.append("附录")
        lines.append("-" * 30)
        lines.append("A. 检测标准: 管孔直径标准 17.73mm ±0.07/-0.05mm")
