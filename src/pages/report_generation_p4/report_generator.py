...
class ReportGenerator:
    ...
    def __init__(self, data_root_path: str = "Data/CAP1000"):
        ...
        self.upper_tolerance = 0.07    # mm
        self.lower_tolerance = 0.05    # mm
    ...
    def collect_workpiece_data(self, workpiece_id: str) -> ReportData:
        ...
        # 根据实际收集到的孔位数据生成工件基本信息
        workpiece_info = self._collect_workpiece_info(workpiece_id, all_hole_data)
    ...
    def _collect_workpiece_info(self, workpiece_id: str, hole_data: List[HoleQualityData] = None) -> WorkpieceInfo:
        """收集工件基本信息"""
        # 根据工件ID返回相应的工件信息
        if workpiece_id == "CAP1000":
            # 动态生成描述文本
            description = "管孔检测系统工件，包含20050个孔位"
            if hole_data:
                hole_ids = [hole.hole_id for hole in hole_data]
                if hole_ids:
                    hole_ids.sort()  # 排序孔位ID
                    if len(hole_ids) == 1:
                        description += f"，目前已有{hole_ids[0]}孔位的检测数据"
                    elif len(hole_ids) <= 5:
                        description += f"，目前已有{', '.join(hole_ids)}孔位的检测数据"
                    else:
                        description += f"，目前已有{hole_ids[0]}~{hole_ids[-1]}等{len(hole_ids)}个孔位的检测数据"
            ...
            return WorkpieceInfo(
                ...
                type="管板工件",
                material="母材材质：SA508.Gr3. C1.2；堆焊层材质：镍基堆焊层",
                total_holes=len(hole_data) if hole_data else 48,  # 使用实际孔位数量
                description=description,
                created_at=datetime.now()
            )
    ...
    def _collect_hole_quality_data(self, hole_id: str, hole_dir: Path) -> Optional[HoleQualityData]:
        ...
        try:
            ...
            # 查找最新的测量数据文件，支持多种文件名格式
            csv_files = []

            # 尝试多种文件名模式
            patterns = [
                "measurement_data_*.csv",  # 标准格式
                f"{hole_id}.csv",          # 孔位ID格式
                "R*_C*.csv",               # R0_C0格式
                "*.csv"                    # 通用CSV文件
            ]

            for pattern in patterns:
                found_files = list(ccidm_dir.glob(pattern))
                if found_files:
                ...
                try:
                    with open(latest_csv, 'r', encoding=encoding) as f:
                        ...
                        for i, col_name in enumerate(header):
                            if any(keyword in col_name.lower() for keyword in ['直径', 'diameter', '管孔']):
                        ...
                        for row in reader:
                            try:
                                if len(row) > abs(diameter_col_index):
                                    diameter_str = row[diameter_col_index].strip()
                                    if diameter_str:  # 确保不是空字符串
    ...
    def _estimate_hole_position(self, hole_id: str) -> Tuple[float, float]:
        ...
        try:
            # 对于新格式ID（如R001C001）
            if hole_id.startswith('R') and 'C' in hole_id:
                # 提取行号和列号
                parts = hole_id.split('C')
                if len(parts) == 2:
                    row_part = parts[0][1:]  # 去掉'R'
                    col_part = parts[1]
