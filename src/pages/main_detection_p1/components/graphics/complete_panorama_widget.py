...
class CompletePanoramaWidget(QWidget):
    ...
    def _adjust_hole_display_size(self):
        ...
        try:
            ...
            # 应用用户自定义缩放（解决孔位重叠问题）
            user_scale_factor = getattr(self, '_user_hole_scale_factor', 0.1)  # 默认10%大小
            adjusted_radius = display_radius * global_scale_factor * user_scale_factor
            
            self.logger.info(f"数据规模:{data_scale}, 孔位:{hole_count}, 密度:{density:.6f}, "
                           f"基础半径:{display_radius:.2f}px, 全局系数:{global_scale_factor:.2f}, "
                           f"用户缩放:{user_scale_factor*100:.1f}%, 最终半径:{adjusted_radius:.2f}px", "🔍")
    ...
    def _create_sector_highlights(self):
        ...
        self.logger.info(f"扇形高亮项创建完成: {success_count}/{len(SectorQuadrant)} 个", "✅")
    ...
    def _create_sector_dividers(self):
        ...
        try:
            ...
            # 创建明显的十字分隔线
            pen = QPen(QColor(80, 80, 80, 200))  # 深灰色，更不透明
            pen.setWidth(3)  # 增加线宽
            pen.setStyle(Qt.SolidLine)  # 实线更明显
    ...
    def _execute_batch_update(self):
        ...
        # 只在批量更新数量超过阈值时记录日志，减少垃圾信息
        if update_count > 10:
            self.logger.info(f"开始批量更新 {update_count} 个孔位状态", "🚀")
        else:
            self.logger.debug(f"小量更新 {update_count} 个孔位状态", "🚀")
        ...
        try:
            ...
            if hasattr(self.panorama_view, 'hole_items'):
                ...
                if update_count > 10:
                    ...
                    self.logger.debug(f"小量更新完成: {updated_count}/{update_count} 个孔位", "✅")
    ...
    def set_user_hole_scale_factor(self, scale_factor: float):
        """
        设置用户自定义孔位缩放因子（解决孔位重叠问题）
        
        Args:
            scale_factor: 缩放因子，0.1表示10%大小，0.01表示1%大小
        """
        self._user_hole_scale_factor = max(0.001, min(1.0, scale_factor))  # 限制在0.1%-100%之间
        self.logger.info(f"🔧 设置用户孔位缩放因子: {self._user_hole_scale_factor*100:.1f}%")
        
        # 如果已经加载了数据，重新调整孔位大小
        if hasattr(self, 'hole_collection') and self.hole_collection:
            self._smart_adjust_hole_display_size()
    
    def get_user_hole_scale_factor(self) -> float:
        """获取当前的用户孔位缩放因子"""
        return getattr(self, '_user_hole_scale_factor', 0.1)