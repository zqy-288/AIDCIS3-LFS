"""
增强版工件示意图组件
在原WorkpieceDiagram基础上增加了从HoleCollection加载数据的能力
支持从DXF文件加载真实数据并自动进行坐标转换
"""
...
# 导入必要的模型 - 使用简化的占位类以避免复杂依赖
class HoleCollection:
    def __init__(self, holes=None):
        self.holes = holes or {}
        
class HoleData:
    def __init__(self, hole_id=None, center_x=0, center_y=0, radius=8):
...
class EnhancedWorkpieceDiagram(QWidget):
    """工件示意图组件"""
    ...
    def load_hole_collection(self, hole_collection: HoleCollection):
        ...
        if not hole_collection:
            logging.warning("load_hole_collection: 收到空的hole_collection")
            return
        ...
        logging.info(f"开始加载 {hole_count} 个孔位到WorkpieceDiagram")
        
        if hole_count == 0:
            logging.warning("hole_collection中没有孔位数据")
            return
        ...
        # 获取所有孔位的边界
        holes_dict = hole_collection.holes if hasattr(hole_collection, 'holes') else {}
        for hole_data in holes_dict.values():
            if hasattr(hole_data, 'center_x') and hasattr(hole_data, 'center_y'):
                min_x = min(min_x, hole_data.center_x)
                max_x = max(max_x, hole_data.center_x)
                min_y = min(min_y, hole_data.center_y)
                max_y = max(max_y, hole_data.center_y)
        
        # 添加工件外框（基于数据边界）
        margin = 50
        workpiece_rect = QRectF(min_x - margin, min_y - margin, 
        ...
        # 添加标题
        title_item = QGraphicsTextItem("DXF工件视图")
        title_item.setPos((min_x + max_x) / 2 - 40, min_y - margin - 30)
        font = QFont("Arial", 12, QFont.Bold)
        title_item.setFont(font)
        self.graphics_scene.addItem(title_item)
        
        # 创建检测点
        for hole_id, hole_data in holes_dict.items():
            if hasattr(hole_data, 'center_x') and hasattr(hole_data, 'center_y'):
                # 使用hole_data的实际半径，如果没有则使用默认值
                radius = getattr(hole_data, 'radius', 8)
                
                # 创建检测点
                point = DetectionPoint(hole_id, hole_data.center_x, hole_data.center_y, radius)
                
                # 如果hole_data有状态信息，设置状态
                if hasattr(hole_data, 'status'):
                    # 将状态字符串转换为枚举
                    status_map = {
                        'pending': DetectionStatus.NOT_DETECTED,
                        'qualified': DetectionStatus.QUALIFIED,
                        'defective': DetectionStatus.UNQUALIFIED,
                        'detecting': DetectionStatus.DETECTING,
                        'real_data': DetectionStatus.REAL_DATA
                    }
                    status_str = getattr(hole_data.status, 'value', str(hole_data.status))
                    if status_str in status_map:
                        point.set_status(status_map[status_str])
        ...
        logging.info(f"成功加载 {len(self.detection_points)} 个孔位")
    ...
    def zoom_in(self):
        """放大视图 - 使用简化统一接口"""
        from src.core_business.graphics.simple_unified_interface import get_simple_unified_interface
        get_simple_unified_interface().zoom_in("workpiece_diagram", 1.2)
        
    def zoom_out(self):
        """缩小视图 - 使用简化统一接口"""
        from src.core_business.graphics.simple_unified_interface import get_simple_unified_interface
        get_simple_unified_interface().zoom_out("workpiece_diagram", 0.8)
        
    def reset_zoom(self):
        """重置缩放 - 使用简化统一接口"""
        from src.core_business.graphics.simple_unified_interface import get_simple_unified_interface
        get_simple_unified_interface().reset_zoom("workpiece_diagram")
...
if __name__ == "__main__":
    ...
    # 创建工件示意图
    diagram = EnhancedWorkpieceDiagram()
