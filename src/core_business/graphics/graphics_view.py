...
class OptimizedGraphicsView(InteractionMixin, NavigationMixin, QGraphicsView):
    ...
    def update_hole_status(self, hole_id: str, status: HoleStatus):
        """更新孔状态"""
        print(f"[DEBUG GraphicsView] update_hole_status: hole_id={hole_id}, status={status}")
        
        if hole_id in self.hole_items:
            hole_item = self.hole_items[hole_id]
            print(f"[DEBUG GraphicsView] 找到孔位图形项: {hole_id}")
            
            # 更新状态
            old_status = hole_item.hole_data.status
            hole_item.update_status(status)
            print(f"[DEBUG GraphicsView] 孔位 {hole_id} 状态: {old_status} -> {status}")
            ...
            # 强制刷新场景区域
            item_rect = hole_item.sceneBoundingRect()
            ...
            print(f"[DEBUG GraphicsView] 孔位 {hole_id} 刷新完成")
        else:
            print(f"[DEBUG GraphicsView] 未找到孔位图形项: {hole_id}")
            print(f"[DEBUG GraphicsView] 现有孔位数: {len(self.hole_items)}")
            # 打印前10个孔位 ID
            sample_ids = list(self.hole_items.keys())[:10]
            print(f"[DEBUG GraphicsView] 示例孔位 IDs: {sample_ids}")
    
    def batch_update_status(self, status_updates: Dict[str, HoleStatus]):
        """批量更新状态"""
        print(f"[DEBUG GraphicsView] batch_update_status: 更新 {len(status_updates)} 个孔位")
