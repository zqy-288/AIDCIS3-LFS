...
from src.shared.services.cache.business_cache_manager import BusinessCacheManager, cached_business_operation
from src.shared.services.business_rules.rule_engine import BusinessRuleEngine, apply_business_rules
...
class DataAdapter:
    ...
    def __init__(self, 
        ...
        # 确保获取DataAccessLayer实例
        if data_access_layer is None:
            from src.core.dependency_injection import get_container
            try:
                container = get_container()
                self.data_access_layer = container.resolve(DataAccessLayer)
                self.logger.info("DataAccessLayer从容器中解析成功")
            except Exception as e:
                self.logger.error(f"DataAccessLayer解析失败: {e}")
    ...
    def _get_repository(self, repository_class):
        ...
        try:
            if self.data_access_layer is None:
                self.logger.warning(f"DataAccessLayer不可用，无法获取Repository: {repository_class.__name__}")
                return None
            
            result = self.data_access_layer.get_repository(repository_class.__name__)
            if result is None:
                self.logger.info(f"Repository {repository_class.__name__} 尚未实现，使用默认处理")
            return result
        except Exception as e:
