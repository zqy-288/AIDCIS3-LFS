...
class ToolbarComponent(QFrame):
    ...
    snake_path_toggled = Signal(bool)  # enabled state
    snake_path_strategy_changed = Signal(str)  # strategy name
    snake_path_debug_toggled = Signal(bool)  # debug state
    ...
    def __init__(self, parent: Optional = None):
        ...
        self.snake_path_checkbox: Optional[QCheckBox] = None
        self.snake_path_strategy_combo: Optional[QComboBox] = None
        self.snake_path_debug_checkbox: Optional[QCheckBox] = None
    ...
    def _setup_ui(self) -> None:
        ...
        # Snake path controls
        snake_path_label = QLabel("路径显示:")
        snake_path_label.setFont(toolbar_font)
        
        self.snake_path_checkbox = QCheckBox("显示蛇形路径")
        self.snake_path_checkbox.setFont(toolbar_font)
        self.snake_path_checkbox.setChecked(False)
        
        self.snake_path_strategy_combo = QComboBox()
        self.snake_path_strategy_combo.addItems(["混合策略", "标签策略", "空间策略"])
        self.snake_path_strategy_combo.setMinimumHeight(35)
        self.snake_path_strategy_combo.setFont(toolbar_font)
        self.snake_path_strategy_combo.setEnabled(False)
        
        self.snake_path_debug_checkbox = QCheckBox("调试")
        self.snake_path_debug_checkbox.setFont(toolbar_font)
        self.snake_path_debug_checkbox.setChecked(False)
        
        layout.addWidget(snake_path_label)
        layout.addWidget(self.snake_path_checkbox)
        layout.addWidget(self.snake_path_strategy_combo)
        layout.addWidget(self.snake_path_debug_checkbox)
    ...
    def _connect_signals(self) -> None:
        ...
        try:
            ...
            # Snake path controls
            if self.snake_path_checkbox:
                self.snake_path_checkbox.toggled.connect(self._on_snake_path_toggled)
            
            if self.snake_path_strategy_combo:
                self.snake_path_strategy_combo.currentTextChanged.connect(
                    self._on_snake_path_strategy_changed
                )
            
            if self.snake_path_debug_checkbox:
                self.snake_path_debug_checkbox.toggled.connect(
                    lambda checked: self.snake_path_debug_toggled.emit(checked)
                )
    ...
    def _on_snake_path_toggled(self, checked: bool) -> None:
        """Handle snake path checkbox toggle."""
        # Enable/disable strategy combo based on checkbox state
        if self.snake_path_strategy_combo:
            self.snake_path_strategy_combo.setEnabled(checked)
        
        self.snake_path_toggled.emit(checked)
        self.logger.debug(f"Snake path toggled: {checked}")
    
    def _on_snake_path_strategy_changed(self, text: str) -> None:
        """Handle snake path strategy change."""
        # Map Chinese text to strategy types
        strategy_map = {
            "混合策略": "hybrid",
            "标签策略": "label", 
            "空间策略": "spatial"
        ...
        strategy = strategy_map.get(text, "hybrid")
        self.snake_path_strategy_changed.emit(strategy)
        self.logger.debug(f"Snake path strategy changed to: {strategy}")
    ...
    def update_from_view_model(self, view_model: MainViewModel) -> None:
        ...
        try:
            ...
            # Update snake path controls
            if self.snake_path_checkbox:
                if self.snake_path_checkbox.isChecked() != view_model.snake_path_enabled:
                    self.snake_path_checkbox.setChecked(view_model.snake_path_enabled)
            ...
            if self.snake_path_strategy_combo:
                ...
                display_text = strategy_map.get(view_model.snake_path_strategy, "混合策略")
                current_text = self.snake_path_strategy_combo.currentText()
                if display_text != current_text:
                    self.snake_path_strategy_combo.setCurrentText(display_text)
                
                # Ensure combo is enabled/disabled based on checkbox
                self.snake_path_strategy_combo.setEnabled(view_model.snake_path_enabled)
            
            if self.snake_path_debug_checkbox:
                if self.snake_path_debug_checkbox.isChecked() != view_model.snake_path_debug:
                    self.snake_path_debug_checkbox.setChecked(view_model.snake_path_debug)
    ...
    def set_enabled(self, enabled: bool) -> None:
        ...
        if self.snake_path_checkbox:
            self.snake_path_checkbox.setEnabled(enabled)
        if self.snake_path_strategy_combo:
            # Only enable if snake path is enabled and toolbar is enabled
            checkbox_enabled = self.snake_path_checkbox.isChecked() if self.snake_path_checkbox else False
            self.snake_path_strategy_combo.setEnabled(enabled and checkbox_enabled)
        if self.snake_path_debug_checkbox:
            self.snake_path_debug_checkbox.setEnabled(enabled)
