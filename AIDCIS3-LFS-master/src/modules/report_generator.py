...
# 不再使用HoleIdMapper，直接解析CxxxRxxx格式
...
class ReportGenerator:
    ...
    def _collect_workpiece_info(self, workpiece_id: str) -> WorkpieceInfo:
        ...
        elif workpiece_id.startswith('C') and 'R' in workpiece_id:
            # 单个孔位的情况（CxxxRxxx格式）
            return WorkpieceInfo(
                workpiece_id=workpiece_id,
    ...
    def _collect_all_hole_quality_data(self, workpiece_id: str) -> List[HoleQualityData]:
        ...
        # 对于单个孔位ID（如C001R001），直接处理该孔位
        if workpiece_id.startswith('C') and 'R' in workpiece_id:
            ...
            # 直接扫描Data目录下的所有新格式孔位目录（C开头且包含R）
            for hole_dir in self.data_root_path.iterdir():
                if hole_dir.is_dir() and hole_dir.name.startswith('C') and 'R' in hole_dir.name:
    ...
    def _estimate_hole_position(self, hole_id: str) -> Tuple[float, float]:
        ...
        try:
            # 解析CxxxRxxx格式ID
            if hole_id.startswith('C') and 'R' in hole_id:
                # 分割C和R部分
                parts = hole_id[1:].split('R')  # 去掉C前缀，按R分割
                if len(parts) == 2:
                    col = int(parts[0])  # C后面的数字
                    row = int(parts[1])  # R后面的数字
    ...
    def _collect_images_data(self, workpiece_id: str) -> Dict[str, str]:
        ...
        if workpiece_id.startswith('C') and 'R' in workpiece_id:
            ...
            # 对于工件ID，扫描Data目录下所有C开头的孔位目录（CxxxRxxx格式）
            for hole_dir in self.data_root_path.iterdir():
                if hole_dir.is_dir() and hole_dir.name.startswith('C') and 'R' in hole_dir.name:
                    hole_id = hole_dir.name
