...
# 创建一个简单的SectorProgress类用于向后兼容
class SectorProgress:
    def __init__(self, sector: SectorQuadrant, total_holes: int = 0, completed_holes: int = 0, 
                 qualified_holes: int = 0, defective_holes: int = 0, progress_percentage: float = 0.0, 
                 status_color=None):
        self.sector = sector
        self.total_holes = total_holes
        self.completed_holes = completed_holes
        self.qualified_holes = qualified_holes
        self.defective_holes = defective_holes
...
class SectorGraphicsItem(QGraphicsPathItem):
    ...
    def __init__(self, sector: SectorQuadrant, center: QPointF, radius: float, parent=None):
        ...
        self.span_angle = self._get_span_angle()  # 支持自适应角度
    ...
    def _get_start_angle(self) -> float:
        ...
        优先使用自适应角度计算，如果不可用则使用默认角度
        """
        # 尝试从父组件获取自适应角度配置
        adaptive_angles = self._get_adaptive_angle_config()
        
        if adaptive_angles:
            return adaptive_angles.get('start_angle', self._get_default_start_angle())
        else:
            return self._get_default_start_angle()
    
    def _get_default_start_angle(self) -> float:
        """获取默认扇形起始角度
        ...
        """
        angle_map = {
            SectorQuadrant.SECTOR_1: 315,   # 右上角：从315°开始
            SectorQuadrant.SECTOR_2: 45,    # 左上角：从45°开始
            SectorQuadrant.SECTOR_3: 135,   # 左下角：从135°开始
            SectorQuadrant.SECTOR_4: 225    # 右下角：从225°开始
        }
        return angle_map[self.sector]
    
    def _get_adaptive_angle_config(self) -> Optional[Dict[str, float]]:
        """获取自适应角度配置"""
        # 尝试从场景的父组件获取自适应角度配置
        scene = self.scene()
        if scene and hasattr(scene, 'parent_widget'):
            parent_widget = scene.parent_widget
            
            # 检查是否有增强型扇形管理器
            if hasattr(parent_widget, 'enhanced_sector_manager'):
                manager = parent_widget.enhanced_sector_manager
                if manager and hasattr(manager, 'get_sector_angle_config'):
                    return manager.get_sector_angle_config(self.sector)
            
            # 检查是否有传统扇形管理器
            if hasattr(parent_widget, 'sector_manager'):
                manager = parent_widget.sector_manager
                if manager and hasattr(manager, 'get_sector_angle_config'):
                    return manager.get_sector_angle_config(self.sector)
        
        return None
    
    def _get_span_angle(self) -> float:
        """获取扇形跨度角度"""
        # 尝试从自适应角度配置获取
        adaptive_angles = self._get_adaptive_angle_config()
        
        if adaptive_angles:
            return adaptive_angles.get('span_angle', 90.0)  # 默认90度
        else:
            return 90.0  # 默认90度
...
class SectorOverviewWidget(QWidget):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.sector_manager: Optional['UnifiedSectorAdapter'] = None
    ...
    def set_sector_manager(self, sector_manager: 'UnifiedSectorAdapter'):
        """设置扇形管理器"""
...
class SectorDetailView(QWidget):
    ...
    def setup_ui(self):
        ...
        try:
            ...
            self.hole_list_widget.setStyleSheet("border: 1px solid #ccc; padding: 8px; color: #D3D8E0;")
    ...
    def update_sector_display(self, sector: SectorQuadrant, progress: SectorProgress):
        ...
        sector_names = {
            ...
            SectorQuadrant.SECTOR_3: "区域4 (左下)",
            SectorQuadrant.SECTOR_4: "区域3 (右下)"
        }
